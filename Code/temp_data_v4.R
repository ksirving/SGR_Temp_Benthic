...
df_date <- df %>%
  ...
  full_join(airtempx, by = "Date", keep = T)  %>% 
  rename(Date = Date.x) %>% select(-Date.y) %>%
  ...
  mutate(Metric = factor(Metric, levels = c("max07da", "mn07da", "MeanTemp", "MinTemp", "MaxTemp", "DTR"))) %>%
  mutate(SiteName = factor(SiteName, levels = c("Burbank", "Rattlesnake", "Benedict", "Steelhead", "Riverfront",
                                                "Compton", "Willow")))
...
m1 <- mapview(sites, col.regions = "orange",
              layer.name="Temperature Loggers")  %>%
...
head(df_date_type)
unique(df_date_type$Metric)

df_date_type <- df_date %>%
...
for(s in 1:length(sites)) {
  ...
  fitted_datax$PVal <- mod_sum$coefficients[4] ## p value
...
for(s in 1:length(sites)) {
  ...
  STfitted_datax$PVal <- mod_sum$coefficients[4] ## p value
...
file.name1 <- paste0(out.dir, "Temp_vs_air_per_metric_fitted_streamtyoe.jpg")
...
T5a <- ggplot(STfitted_data, aes(y = WaterTempFit, x = AirTemp, group = StreamType, color = StreamType)) +
  stat_smooth(method = "lm") +
  # facet_wrap(~StreamType) + ## , scales = "free_x"
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  scale_color_brewer(palette= 'Dark2', direction=-1) +
